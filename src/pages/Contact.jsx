...
const Icon = ({ name, className }) => {
    const icons = {
        ...
        headquarter: (
            <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0v-4m0 4h5m0 0v-4m0 4H3m3-4h3m-3 0V7h3v10m0 0v-4" />
            </svg>
        ),
        hours: (
            <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const InfoCard = ({ iconName, title, lines }) => (
    <div className="flex items-start space-x-4 p-4">
        ...
        <div>
            <h3 className="font-bold text-sm tracking-widest text-gray-500 uppercase">{title}</h3>
    ...
    return (
        <div className="bg-white py-16 sm:py-24">
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tighter">SEND US FEEDBACK</h2>
                ...
                {submitted ? (
                    ...
                    <form onSubmit={handleSubmit} className="mt-12 text-left">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Your Name *" required className="p-4 bg-gray-100 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-gray-800 transition" />
                            <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="Phone Number *" required className="p-4 bg-gray-100 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-gray-800 transition" />
                        </div>
                        <div className="mt-6">
                            <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Email *" required className="p-4 bg-gray-100 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-gray-800 transition" />
                        </div>
                        <div className="mt-6">
                            <textarea name="message" value={formData.message} onChange={handleChange} placeholder="Say something..." rows="6" className="p-4 bg-gray-100 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-gray-800 transition"></textarea>
                        </div>
                        <div className="mt-8 text-center">
                            <button type="submit" className="px-10 py-4 bg-gray-900 text-white font-semibold rounded-lg hover:bg-gray-700 transition-colors duration-300">
                                Submit
                            </button>
                        </div>
                    </form>
